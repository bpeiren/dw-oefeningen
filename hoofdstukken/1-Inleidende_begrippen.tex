...
\begin{enumerate}
  ...
  \item \(p(x) = ``x \text{ is deelbaar door 10}''\) en \(q(x) = ``x \text{ is deelbaar door 2}''\)
    \begin{enumerate}[label=\alph*)]
        ...
        \item Conjunctie: \(p(x) \land q(x)\): \(x \mod 10 = 0 \land x \mod 2 = 0\). Dit reduceert tot \(x \mod 10 = 0\) omdat \(x \mod 10 = 0 \implies x \mod 2 = 0\).
        \item Implicatie \(q(x)\implies p(x)\): \(x \mod 2 = 0 \Rightarrow x \mod 10 = 0\). \\
                Contrapositie: \(x \mod 10 \ne 0 \Rightarrow x \mod 2 \ne 0\).
        \item Equivalentie \(p(x)\Leftrightarrow q(x)\): \(x \mod 10 = 0 \Leftrightarrow x \mod 2 = 0\).
    \end{enumerate}
    Waarheidswaarden:
    \begin{itemize}
        \item \(q(x)\implies p(x)\) is \textbf{onwaar}: neem \(x=2\) dan \(2 \mod 2 = 0\) maar \(2 \mod 10 \ne 0\).
        ...
        \item Equivalentie \(p(x)\Leftrightarrow q(x)\) is onwaar: \(x=2\) is tegenvoorbeeld (rechts waar, links onwaar); ook \(x=10\) maakt beide waar maar één voorbeeld volstaat om onwaarheid aan te tonen.
    \end{itemize}
  ...
  \item \((p \implies q) \implies (q \implies p)\):
     ...
     \begin{array}{c c | c c | c}
       p & q & p\implies q & q\implies p & (p\implies q)\implies(q\implies p) \\\hline
       T & T & T & T & T \\
       T & F & F & T & T \\
       F & T & T & F & F \\
       F & F & T & T & T \\
  ...
  \item \([(p\implies q) \land (q\implies r)] \implies (p \implies r)\) (transitiviteit van implicatie):
     \[
     \begin{array}{c c c | c c | c | c | c}
       p & q & r & p\implies q & q\implies r & (p\implies q)\land(q\implies r) & p\implies r & \text{geheel} \\\hline
       T & T & T & T & T & T & T & T \\
       T & T & F & T & F & F & F & T \\
       T & F & T & F & T & F & T & T \\
       T & F & F & F & T & F & F & T \\
  ...
  % Oefening 5
  \item Toon aan dat \(p \Leftrightarrow q\) en \((p \implies q) \land (q \implies p)\) logisch equivalent zijn.
    \[
    \begin{array}{c c | c | c c | c}
        p & q & p \Leftrightarrow q & p \implies q & q \implies p & (p \implies q) \land (q \implies p) \\\hline
        T & T & T & T & T & T \\
        T & F & F & F & T & F \\
        F & T & F & T & F & F \\
        F & F & T & T & T & T \\
    \end{array}
    \]
    De kolom \(p \Leftrightarrow q\) is identiek aan \((p \implies q) \land (q \implies p)\), dus ze zijn logisch equivalent.
